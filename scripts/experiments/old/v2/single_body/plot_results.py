...
CONSTRAINT_LABELS = ["Nom", "Ell", "Ell+CoM", "CHE", "Disc"]
...
def parse_pickle(path):
    ...
    disc_geo_err = np.array(data["discretized"]["riemannian_errors"])
    ...
    disc_val_err = np.array(data["discretized"]["validation_errors"])
    ...
    disc_solve_times = 1000 * np.array(data["discretized"]["solve_times"])
    ...
    disc_num_feas = 100 * data["discretized"]["num_feasible"] / n
...
def make_boxplot(ax, data, positions, bar_width=BAR_WIDTH):
    ...
    for i in range(NUM_SCENARIOS):
        boxes = bplot["boxes"][
            i * NUM_CONSTRAINT_TYPES : (i + 1) * NUM_CONSTRAINT_TYPES
        ]
...
def geodesic_bar_data(ax, results, bar_width=BAR_WIDTH):
    ...
    locs = np.array([-2, -1, 0, 1, 2]) * bar_width
    ...
    for result in results:
        ...
        data.append(result["disc_geo_err"])
...
def validation_bar_data(ax, results, bar_width=BAR_WIDTH):
    ...
    for result in results:
        ...
        data.append(result["disc_val_err"])
    positions = np.concatenate([x + locs for x in label_xs])
    return make_boxplot(ax, data, positions, bar_width=bar_width)
...
def solve_time_bar_data(ax, results, bar_width=BAR_WIDTH):
    ...
    for result in results:
        ...
        data.append(result["disc_solve_times"])
    positions = np.concatenate([x + locs for x in label_xs])
    return make_boxplot(ax, data, positions, bar_width=bar_width)
...
def feasibility_bar_data(ax, results, bar_width=BAR_WIDTH):
    ...
    poly_num_feas = [result["disc_num_feas"] for result in results]
