...
def test_random_points_on_hypersphere():
    ...
    # one point
    point = rg.random_points_on_hypersphere(dim=2)
    assert point.shape == (3,)
    assert np.isclose(np.linalg.norm(point), 1.0)

    # multiple points
    points = rg.random_points_on_hypersphere(shape=10, dim=2)
    assert points.shape == (10, 3)
    assert np.allclose(np.linalg.norm(points, axis=-1), 1.0)

    # hypersphere
    points = rg.random_points_on_hypersphere(shape=10, dim=3)
    assert np.allclose(np.linalg.norm(points, axis=-1), 1.0)

    # grid of points
    points = rg.random_points_on_hypersphere(shape=(10, 10), dim=2)
    assert points.shape == (10, 10, 3)
    assert np.allclose(np.linalg.norm(points, axis=-1), 1.0)

    # grid with one dimension 1
    points = rg.random_points_on_hypersphere(shape=(10, 1), dim=2)
...
def test_random_points_in_ball():
    ...
    # one point
    point = rg.random_points_in_ball(dim=3)
    assert point.shape == (3,)
    assert np.linalg.norm(point) <= 1.0

    # multiple points
    points = rg.random_points_in_ball(shape=10, dim=3)
    assert points.shape == (10, 3)
    assert np.all(np.linalg.norm(points, axis=-1) <= 1.0)

    # hyperball
    points = rg.random_points_in_ball(shape=10, dim=4)
    assert points.shape == (10, 4)
    assert np.all(np.linalg.norm(points, axis=-1) <= 1.0)

    # grid of points
    points = rg.random_points_in_ball(shape=(10, 10), dim=3)
    assert points.shape == (10, 10, 3)
    assert np.all(np.linalg.norm(points, axis=-1) <= 1.0)

    # grid with one dimension 1
    points = rg.random_points_in_ball(shape=(10, 1), dim=3)
    assert points.shape == (10, 1, 3)
    assert np.all(np.linalg.norm(points, axis=-1) <= 1.0)
    ...
    n = 10000
    points = rg.random_points_in_ball(shape=n, dim=3)
